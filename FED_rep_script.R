...
#  (2) Illiquid MPC — rolling (no COVID dummy)
#  (3) Liquid MPC — rolling (+ COVID dummy intercept)
#  (4) Illiquid MPC — rolling (+ COVID dummy intercept)
# Data: sheet "variables", date col "Datei t" (col A). ALL series already REAL.
# Dep var: C / (Y - T). Coefficients shown as cents per £1 (100 × beta).

suppressPackageStartupMessages({
  library(readxl); library(dplyr); library(ggplot2); library(zoo)
})

# ---- CONFIG: change the windows here ----
ROLL_WINS <- c(20L, 25L, 30L)   # <- edit this to try e.g. c(10L,15L,20L) or any set you like
...
COL_MS <- "Real Mortgage Spread"   # ALWAYS included

# COVID dummy window (intercept shift only)
COVID_START <- as.yearqtr("2020 Q1")
COVID_END   <- as.yearqtr("2021 Q4")

# ---- Dynamic colours/labels based on ROLL_WINS ----
labs_windows <- paste0(ROLL_WINS, "q")
pal <- grDevices::hcl.colors(length(ROLL_WINS), "Dark 3")
col_map  <- setNames(pal, labs_windows)
fill_map <- setNames(pal, labs_windows)   # transparency set in ribbon
win_order <- labs_windows
...
# Rolling OLS → coefficient and OLS SE for chosen regressor (liq or ill), ALWAYS incl. spread
roll_coeff <- function(df, win, target = c("liq","ill"), with_dummy = FALSE) {
  ...
  for (s in 1:(n - win + 1L)) {
    ...
    fml <- if (with_dummy) {
      ratio_data ~ Wliq_den + Williq_den + MSpr + Dcovid   # intercept shift only
    } else {
  ...
  dplyr::bind_rows(out) |>
    dplyr::mutate(Window = paste0(win, "q"),
                  beta_cents = 100 * beta,
                  lo = 100 * (beta - 1.96 * se),
                  hi = 100 * (beta + 1.96 * se)) |>
    dplyr::arrange(Date)
...
# Panel builder with inline legend (colored bars + labels in top-right)
  ...
  ggplot() +
    ...
    { if (nW > 0)
        geom_text(data = legend_df,
                  ...
                  inherit.aes = FALSE) } +
    scale_colour_manual(values = col_map, breaks = win_order) +
    scale_fill_manual(values = fill_map,  breaks = win_order) +
...
# ---- Load & prepare (ALL series already REAL; no deflator) ----
raw <- read_excel(XLSX_FILE, sheet = SHEET, skip = 0)
names(raw) <- trimws(names(raw))

# Parse "Datei t" to quarters → Date
x  <- raw[[DATE_COL]]
...
# ---- Compute rolling series for each window (ALWAYS incl. spread) ----
...
p1 <- make_panel(L1_lines, L1_bands, paste0("Liquid MPC — rolling (", paste(labs_windows, collapse="/"), ", no COVID dummy; incl. spread)"))
p2 <- make_panel(L2_lines, L2_bands, paste0("Illiquid MPC — rolling (", paste(labs_windows, collapse="/"), ", no COVID dummy; incl. spread)"))
p3 <- make_panel(L3_lines, L3_bands, paste0("Liquid MPC — rolling (", paste(labs_windows, collapse="/"), ", + COVID dummy; incl. spread)"))
p4 <- make_panel(L4_lines, L4_bands, paste0("Illiquid MPC — rolling (", paste(labs_windows, collapse="/"), ", + COVID dummy; incl. spread)"))

# ---- Combine in a 2×2 matrix ----
if (requireNamespace("patchwork", quietly = TRUE)) {
  library(patchwork)
  panel <- (p1 | p2) / (p3 | p4)
  panel  # prints the whole panel
} else {
  grid::grid.newpage()
...
# ---------------- Notes ----------------
# • To try different windows, edit ROLL_WINS above (e.g., c(10L,15L,20L) or c(12L,24L,36L)).
# • Lines are timed at the CENTER of each rolling window (so they start at different dates).
# • You need at least max(ROLL_WINS) quarters of non-missing data.
...
# ================= Figure 2 — Rolling MPC (10/15/20q), 2×2 panel — WITH SPREAD =================
# Panels:
#  (1) Liquid MPC — rolling (no COVID dummy)
#  (2) Illiquid MPC — rolling (no COVID dummy)
#  (3) Liquid MPC — rolling (+ COVID dummy intercept)
...
# Rolling OLS → coefficient and OLS SE for chosen regressor (liq or ill), ALWAYS including spread
roll_coeff <- function(df, win, target = c("liq","ill"), with_dummy = FALSE) {
  target <- match.arg(target)
  n <- nrow(df)
  if (n < win) stop("Not enough rows for window ", win, " (n = ", n, ")")
...
# ---- Build the four ggplots with inline legends ----
p1 <- make_panel(L1_lines, L1_bands, "Liquid MPC — rolling (10/15/20q, no COVID dummy; incl. spread)")
p2 <- make_panel(L2_lines, L2_bands, "Illiquid MPC — rolling (10/15/20q, no COVID dummy; incl. spread)")
p3 <- make_panel(L3_lines, L3_bands, "Liquid MPC — rolling (10/15/20q, + COVID dummy; incl. spread)")
p4 <- make_panel(L4_lines, L4_bands, "Illiquid MPC — rolling (10/15/20q, + COVID dummy; incl. spread)")
...
# ================= Figure 2 — Rolling MPC (10/15/20q), 2×2 panel, optional spread =================
# Panels:
#  (1) Liquid MPC — rolling (no COVID dummy)
#  (2) Illiquid MPC — rolling (no COVID dummy)
#  (3) Liquid MPC — rolling (+ COVID dummy intercept)
...
# Toggle: include mortgage spread as a control (TRUE = include; FALSE = omit)
INCLUDE_SPREAD <- TRUE
...
COL_WL <- "Real Liquid Assets"
COL_WI <- "Real Illiquid Assets"
COL_MS <- "Real Mortgage Spread"   # used only if INCLUDE_SPREAD == TRUE
...
roll_coeff <- function(df, win, target = c("liq","ill"), with_dummy = FALSE, include_spread = FALSE) {
  ...
  for (s in 1:(n - win + 1L)) {
    ...
    if (include_spread) {
      ...
      fml <- if (with_dummy) {
        ratio_data ~ Wliq_den + Williq_den + Dcovid
      } else {
...
# No COVID dummy
roll_liq_nod <- lapply(ROLL_WINS, function(w) roll_coeff(d, w, target = "liq", with_dummy = FALSE, include_spread = INCLUDE_SPREAD)) |> dplyr::bind_rows()
roll_ill_nod <- lapply(ROLL_WINS, function(w) roll_coeff(d, w, target = "ill", with_dummy = FALSE, include_spread = INCLUDE_SPREAD)) |> dplyr::bind_rows()
# With COVID dummy (intercept shift only)
roll_liq_cov <- lapply(ROLL_WINS, function(w) roll_coeff(d, w, target = "liq", with_dummy = TRUE,  include_spread = INCLUDE_SPREAD)) |> dplyr::bind_rows()
roll_ill_cov <- lapply(ROLL_WINS, function(w) roll_coeff(d, w, target = "ill", with_dummy = TRUE,  include_spread = INCLUDE_SPREAD)) |> dplyr::bind_rows()
...
# Titles that reflect the toggle
tsuf <- if (INCLUDE_SPREAD) " (incl. mortgage spread)" else ""
p1 <- make_panel(L1_lines, L1_bands, paste0("Liquid MPC — rolling (10/15/20q, no COVID dummy)", tsuf))
p2 <- make_panel(L2_lines, L2_bands, paste0("Illiquid MPC — rolling (10/15/20q, no COVID dummy)", tsuf))
p3 <- make_panel(L3_lines, L3_bands, paste0("Liquid MPC — rolling (10/15/20q, + COVID dummy)", tsuf))
p4 <- make_panel(L4_lines, L4_bands, paste0("Illiquid MPC — rolling (10/15/20q, + COVID dummy)", tsuf))
...
# ================= Figure 2 — Rolling MPC (10, 15, 20q) in a 2×2 panel =================
# Panels:
#  (1) Liquid MPC — rolling (no COVID dummy)
#  (2) Illiquid MPC — rolling (no COVID dummy)
#  (3) Liquid MPC — rolling (+ COVID dummy intercept)
